  ...
  <div class="page-container">
    ...
    <div v-if="roleModal.show" class="modal-overlay" @click="closeRoleModal">
      <div class="modal-content modal-large" @click.stop>
        <h2>{{ roleModal.isEdit ? 'Edit' : 'Create' }} Role</h2>
        <div class="form-group">
          <label>Role Name *</label>
          <input v-model="roleModal.form.name" type="text" required placeholder="e.g., Department Manager" />
        </div>
        <div class="form-group">
        ...
        <div class="modal-actions">
          ...
          <button @click="saveRole" :disabled="roleModal.isLoading" class="btn-primary">
            {{ roleModal.isLoading ? 'Saving...' : 'Save Role' }}
          </button>
        </div>
      </div>
    </div>
    ...
    <div v-if="userRoleModal.show" class="modal-overlay" @click="closeUserRoleModal">
      <div class="modal-content" @click.stop>
        <h2>{{ userRoleModal.user ? 'Change User Role' : 'Assign Role to User' }}</h2>
        <div v-if="userRoleModal.user" class="modal-subtitle">
          <strong>User:</strong> {{ userRoleModal.user.name }} ({{ userRoleModal.user.email }})
        </div>
        ...
        <div class="modal-actions">
          <button @click="closeUserRoleModal" class="btn-secondary">Cancel</button>
          <button @click="saveUserRole" :disabled="userRoleModal.isLoading" class="btn-primary">
            {{ userRoleModal.isLoading ? 'Saving...' : 'Assign Role' }}
          </button>
        </div>
      </div>
    </div>
...
const deleteRole = async (role) => {
  if (['Super Admin', 'Admin'].includes(role.name)) {
    error.value = 'Cannot delete system roles'
