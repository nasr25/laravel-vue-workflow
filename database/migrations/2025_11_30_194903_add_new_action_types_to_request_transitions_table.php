...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::create('request_transitions', function (Blueprint $table) {
            ...
            $table->enum('action', [
                ...
                'assign_path',
                'complete',
                'accept_later',  // New: Accept idea for later implementation
                'reject_idea'    // New: Reject idea (different from workflow reject)
            ]);
    ...
    public function down(): void
    {
        ...
        foreach ($transitions as $transition) {
            if (!in_array($transition->action, ['accept_later', 'reject_idea'])) {
                DB::table('request_transitions')->insert((array) $transition);
            }
        }
    }
